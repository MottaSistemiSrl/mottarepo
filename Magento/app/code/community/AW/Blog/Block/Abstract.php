...
abstract class AW_Blog_Block_Abstract extends Mage_Core_Block_Template
{
    ...
    {
        $more = "<button type='button' class='button btn-readmore' onclick='location.href=\"".$item->getAddress() ."\"'><span><span>Leggi</span></span></button>";
        return $more;
    }
    ...
    protected function _prepareDates($item)
    {
        ...
        $item->setCreatedTime($item->getCreatedTime());
        $item->setUpdateTime($this->formatTime($item->getUpdateTime(), $dateFormat, false));
