...
class AW_Blog_Manage_BlogController extends Mage_Adminhtml_Controller_Action
{
    ...
    protected function uploadFile( $file_name ){
        if( !empty($_FILES[$file_name]['name']) ){
            $result = array();
            $dynamicScmsURL = 'blog' . DS . 'files';
            $baseScmsMediaURL = Mage::getBaseUrl('media') . DS . 'blog' . DS . 'files';
            $baseScmsMediaPath = Mage::getBaseDir('media') . DS .  'blog' . DS . 'files';
    ...
    public function saveAction()
    {
        ...
        // rimuovo i campi che vengono aggiunti dalla tab del prodotti correlati
        // altrimenti il setData() sul model non funziona e l'oggetto non viene salvato
        unset($_POST['page']);
        unset($_POST['limit']);
        unset($_POST['related_product']);
        unset($_POST['id']);
        unset($_POST['name']);
        unset($_POST['sku']);
        unset($_POST['price']);
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if( $files && is_array($files) ){
                for( $f=0; $f<count($files); $f++ ){
                    if( $files[$f] ){
                        ...
                        //	var_dump($fieldname,$files[$f]['url']);
                        //   if( array_key_exists($fieldname, $data) ){
                        $data[$fieldname] = str_replace('\\', '/', $files[$f]['url']);
            ...
            try {
                ...
                if (isset($data['data_evento']) && $data['data_evento']) {
                    $dataEvento = Mage::app()->getLocale()->date($data['data_evento'], Varien_Date::DATE_INTERNAL_FORMAT);
                    $model->setDataEvento($dataEvento->toString('YYYY-MM-dd'));
                }
                ...
                // save related
                $related = Mage::getModel('blog/related');
                foreach($related->getCollection()->addFieldToFilter('post_id', $model->getId()) as $oldAssociation)
                {
                    $oldAssociation->delete()->save();
                }

                foreach($data['related'] as $productId)
                {
                    if(is_numeric($productId))
                    {
                        $related = Mage::getModel('blog/related');
                        $related->setData(array('post_id' => $model->getId(), 'product_id' => $productId));
                        $related->save();
                    }
                }
