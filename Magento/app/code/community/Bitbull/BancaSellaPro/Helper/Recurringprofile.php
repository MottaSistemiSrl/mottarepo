...
class Bitbull_BancaSellaPro_Helper_Recurringprofile extends Bitbull_BancaSellaPro_Helper_Baseclient {

    protected $_webserviceClassName ='bitbull_bancasellapro/webservice_wss2s';
            ...
            if($profile->getState()==Mage_Sales_Model_Recurring_Profile::STATE_PENDING){
                ...
                switch ($webservice->getTransactionResult()){
                    case Bitbull_BancaSellaPro_Model_Webservice_Wscryptdecrypt::TRANSACTION_RESULT_OK :
                        ...
                        /** @var Bitubull_Bancasellapro_Model_Token $tokenInfo */
                        $tokenInfo = Mage::getModel('bitbull_bancasellapro/token');
    ...
    public function getRecurringProfileByQuote($quote){
        ...
        foreach ($quote->getAllVisibleItems()  as $item) {
            if ($item->getProduct()->getIsRecurring()){
                ...
                $recurringItem = $item->getOptionByCode('recurring_profile_options')->getItem();
    ...
    public function payOrderOfProfile($order, $profile){
        ...
        if($token->getId()){
            ...
            if(strcmp($result->getErrorCode(),'8006')==0){
                ...
                Mage::logException(Mage::exception('Bitbull_BancaSellaPro','Account bancasella non correttamente configurato. Le opzioni .'));
                $profile->setState(Mage_Sales_Model_Recurring_Profile::STATE_SUSPENDED);
                $profile->save();
            }else{
                //analizza il risultato
                $helperDecript = Mage::helper('bitbull_bancasellapro/crypt');
