...
class Bitbull_BancaSellaPro_Model_Gestpay extends Mage_Payment_Model_Method_Abstract
    implements Mage_Payment_Model_Recurring_Profile_MethodInterface
{
    ...
    const PAYMENT_INFO_TOKEN = 'bitbull_bancasellapro_payment_token';


    protected $_code= Bitbull_BancaSellaPro_Model_Gestpay::METHOD_CODE;
    protected $_formBlockType = 'bitbull_bancasellapro/form';
    ...
    {
        return Mage::getUrl('bitbull_bancasellapro/gestpay/success',array('_secure' => Mage::app()->getStore()->isCurrentlySecure()));
    }
    ...
    public function getPage3d(){
        return Mage::getUrl('bitbull_bancasellapro/gestpay/confirm3d',array('_secure' => Mage::app()->getStore()->isCurrentlySecure()));
    }
    ...
    {
        return Mage::getUrl('bitbull_bancasellapro/gestpay/redirect', array('_secure' => true));
    }
    ...
    public function assignData($data){
        if($this->isIframeEnable()){
            ...
            if(isset($modulesArray['Bitbull_BancaSellaPro']->depends)){
                $depens = $modulesArray['Bitbull_BancaSellaPro']->depends->children();
                ...
                if(isset($dependsArray['IWD_Opc'])){
                    ...
                    Mage::helper('bitbull_bancasellapro')->log('Salvo l encrypt string per ricreare l\'iframe => '.$encryptString);
                }
            }
        }
    }
    ...
    public function submitRecurringProfile(
        ...
        /** @var Bitubull_Bancasellapro_Model_Token $token */
        $token = Mage::getModel('bitbull_bancasellapro/token')
            ->getCollection()
        ...
        if(!$token->getId()){
            ...
            $order->setState(
                ...
                true,
                Mage::helper('bitbull_bancasellapro')->__('Pagamento iniziale per recupero del token.')
            );
    ...
    protected function _checkAndExecutePayment($recurringProfile){
        ...
        $helperRecurringPayment = Mage::helper('bitbull_bancasellapro/recurringprofile');
    ...
    protected function createOrderAndExecutePayment($recurringProfile, $productItemInfo){
        ...
        Mage::helper('bitbull_bancasellapro/recurringprofile')->payOrderOfProfile($order, $recurringProfile);
    }
    ...
    {
        //ATTUALMENTE E' DISABILITATO L'AGGIORNAMENTO DEI DETTAGLI DEL PAGAMENTO

        //metodo chiamato su update del profilo ricorrente

        //controllare gli ordini è impostare lo stato
...
//        $result->setIsProfileExpired(true)
    ...
    {
        //scegliere se l'utente puo chiedere l'aggiornamento dei dati.
        //bancasella non puo dirci nulla perhcé non gestisce i pagamenti ricorrenti, l'unico problema sarebbe la scadenza del token
        return false;//true;
    }
    ...
    public function updateRecurringProfileStatus(Mage_Payment_Model_Recurring_Profile $profile)
    {
        ...
        Mage::helper('bitbull_bancasellapro')->log('Richiesto '. __METHOD__.' per il profilo'.$profile->getId());

        if($profile->hasNewState()){
            switch ($profile->getNewState()){
                case Mage_Sales_Model_Recurring_Profile::STATE_CANCELED:
                    Mage::helper('bitbull_bancasellapro')->log('Richiesta cancellazione del profilo: '.$profile->getId());
                    break;
                case Mage_Sales_Model_Recurring_Profile::STATE_SUSPENDED:
                    Mage::helper('bitbull_bancasellapro')->log('Richiesta sospensione del profilo: '.$profile->getId());
                    break;
                case Mage_Sales_Model_Recurring_Profile::STATE_ACTIVE:
                    Mage::helper('bitbull_bancasellapro')->log('Richiesta attivazione del profilo: '.$profile->getId());
                    if($profile->getState() == Mage_Sales_Model_Recurring_Profile::STATE_PENDING ){
                        $helper = Mage::helper('bitbull_bancasellapro');
                        Mage::throwException($helper->__('Non è possibile attivare manualmente lo stato, dopo aver effettuato la richiesta di riattivazione'));
                    }
                    $token = Mage::getModel('bitbull_bancasellapro/token')->getFirstValidTokenForProfile($profile);
                    if(!$token->getId()){

                        Mage::helper('bitbull_bancasellapro')->log('Creare un nuovo token, il precedente è scaduto: '.$profile->getId());

                        Mage::app()->getFrontController()->getResponse()->setRedirect(
                            Mage::getUrl('bitbull_bancasellapro/tokenization/newtoken',array('profile'=>$profile->getId()))
                        );
    ...
    protected function _getProductItemInfoToInitialPayment(Mage_Payment_Model_Recurring_Profile $profile){
        ...
        //impostiamo il prezzo iniziale a quello impostato come init oppure al minimo pagabile con bancasella.
        // $init = $init > 0 ? $init : self::MINIMUM_AMOUNT;
    ...
    public function createOrderToNewToken($profile)
    {
        ...
        //inutile creare un pagamento al minimo
//        $productItemInfo = $this->_getProductItemInfoToNewTokenPayment($profile);
