 ...
 */
class Bitbull_BancaSellaPro_Model_Webservice_Wscryptdecrypt extends Bitbull_BancaSellaPro_Model_Webservice_Abstract{
    ...
    public function setOrder(Mage_Sales_Model_Order $order){
        ...
        /**@var $gestpay Bitbull_BancaSellaPro_Model_Gestpay */
    ...
    public function setInfoBeforeOrder(Bitbull_BancaSellaPro_Model_Gestpay $method){
        ...
        if(
            ...
            Mage::helper('bitbull_bancasellapro/recurringprofile')->isRecurringProfile($quote)
        ){
            $recurringProfile = Mage::helper('bitbull_bancasellapro/recurringprofile')->getRecurringProfileByQuote($quote);
            //il primo pagamento deve essere fatto calcolando l'init amount piu il costo della trial/billing_amount

            $amount = $recurringProfile['trial_billing_amount'] > 0 ? $recurringProfile['trial_billing_amount'] :  $recurringProfile['billing_amount'];
            $amount += $recurringProfile['init_amount'];// + $recurringProfile['trial_billing_amount'];

            $amount+= $recurringProfile['shipping_amount']+$recurringProfile['tax_amount'];

            //se il costo è zero allora imposto il minimo per bancasella, questo deve essere fatto perché
            //non si puo creare il token con amount 0, se il costo è impostato a minimum amount allora
            //dovrà essere restituito al cliente
           // $amount = $initalFee>0 ? $initalFee : $recurringProfile['billing_amount'];//Bitbull_BancaSellaPro_Model_Gestpay::MINIMUM_AMOUNT;
