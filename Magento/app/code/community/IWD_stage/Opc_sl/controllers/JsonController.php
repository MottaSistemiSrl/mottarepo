...
class IWD_Opc_JsonController extends Mage_Core_Controller_Front_Action{
	...
	public function saveBillingAction(){
		...
		if ($this->getRequest()->isPost()) {
            ...
            if (!isset($data['amcustomerattr'])){
                $data['amcustomerattr'] = $this->getRequest()->getPost('amcustomerattr', array());
            }
	...
	public function forgotpasswordAction(){
		...
		if ($email) {
			...
			$response['message']  = Mage::helper('customer')->__('Se esiste un account associato a %s riceverai un email con un link per reimpostare la tua password.', Mage::helper('customer')->htmlEscape($email));
			$this->getResponse()->setBody(Mage::helper('core')->jsonEncode($response));
			return;
		} else {
