 ...
 */
/**
 * Previewproduct Adminhtml Controller
 * 
 * @category    Magestore
 * @package     Magestore_PreviewProduct
...
class MicroGiant_PreviewProduct_Adminhtml_PreviewproductController extends Mage_Adminhtml_Catalog_ProductController {
    ...
    public function previewAction() {
        ...
        if ($data) {
            try {
                ...
                $product->setPreviewMode(true);
                $new_product = $product->duplicate();
                
                $new_product = $this->_initProductSavePreview($new_product);
                $new_product->setIsDuplicate(FALSE);
                $new_product->setData('status', Mage_Catalog_Model_Product_Status::STATUS_DISABLED);
                $new_product->setSku('microgiant-preview-'.$new_product->getId());
                $new_product->setIsPreviewMode(true);
                $new_product->save();
                $result['url'] = $new_product->getProductUrl();
                $result['id'] = $new_product->getId();
            } catch (Exception $e) {
                $result['error'] = true;
            }
        } else {
    ...
    protected function _initProductPreview($product)
    {
    ...
    protected function _initProductSavePreview($product)
    {
        $product = $this->_initProductPreview($product);
