    ...
    public function handleBlockOutput($observer) 
    {
        ...
        if ($block instanceof Mage_Checkout_Block_Onepage_Billing) {
            if(!Mage::getSingleton('customer/session')->isLoggedIn()) {
                if (false === strpos($html, 'amcustomerattr')) {
                    $pos = strpos($html, '<li class="fields hidden" id="register-customer-password">');
                    $insert = Mage::helper('amcustomerattr')->fields();
                    $newInsert = str_replace('class="fieldset"', ' class="fieldset forum hiddenDISABLED" ', $insert);
                    $html = substr_replace($html, $newInsert, $pos-1, 0);
                    $transport->setHtml($html);
                }
            }
        }
