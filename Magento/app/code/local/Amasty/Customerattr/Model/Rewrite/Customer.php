...
class Amasty_Customerattr_Model_Rewrite_Customer extends Mage_Customer_Model_Customer
{
    ...
    protected function _beforeSave()
    {
        ...
        */
        ...
        if (!$customerAttributes && ('onepage' == Mage::app()->getRequest()->getModuleName() || 'sgps' == Mage::app()->getRequest()->getModuleName())) {
        ...
        if ($checkUnique) {
            foreach ($checkUnique as $attributeCode => $attributeLabel) {
                ...
                if ($customerCollection->getSize() > 0) {
                    $e = new Mage_Customer_Exception(Mage::helper('amcustomerattr')->__('Specificare un altro valore nel campo `%s`. Un profilo con questo valore già esiste.', $attributeLabel));
                    if (method_exists($e, 'setMessage')) {
                        $e->setMessage(Mage::helper('amcustomerattr')->__('Specificare un altro valore nel campo `%s`. Un profilo con questo valore già esiste.', $attributeLabel));
                    }
                    throw $e;
                }
            }
                ...
                if (isset($_FILES['amcustomerattr_' . $attributeCode]['error']) && UPLOAD_ERR_OK == $_FILES['amcustomerattr_' . $attributeCode]['error']) {
                    ...
                    if ($filesRestrictions[$attributeCode]['dimentions'] && (false !== strpos($_FILES['amcustomerattr_' . $attributeCode]['type'], 'image/'))) {
                        ...
                        try {
                            ...
                            $e = new Mage_Customer_Exception(Mage::helper('amcustomerattr')->__('Errore di sistema: %s', $error));
                            if (method_exists($e, 'setMessage')) {
                                $e->setMessage(Mage::helper('amcustomerattr')->__('Errore di sistema: %s', $error));
                            }
                            throw $e;
                        }
                        $imageInfo = getimagesize($_FILES['amcustomerattr_' . $attributeCode]['tmp_name']);
                        if (($imageInfo[0] > $dimentions[0]) || ($imageInfo[1] > $dimentions[1])) {
                            $fileErrors[] = Mage::helper('amcustomerattr')->__('problema grandezza immagine, grandezza massima consentita %s pixels (altezza/larghezza)', $filesRestrictions[$attributeCode]['dimentions']);
                        }
                    }
                    // errors
                    if ($fileErrors) {
                        $e = new Mage_Customer_Exception(Mage::helper('amcustomerattr')->__('Errore File: %s', implode('; ', $fileErrors)));
                        if (method_exists($e, 'setMessage')) {
                            $e->setMessage(Mage::helper('amcustomerattr')->__('Errore file: %s', implode('; ', $fileErrors)));
                        }
                        throw $e;
                    }
                }
