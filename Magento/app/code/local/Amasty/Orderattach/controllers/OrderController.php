...
class Amasty_Orderattach_OrderController extends Mage_Core_Controller_Front_Action
{
    ...
    {
        $this->getResponse()->setBody($fieldModel->getRenderer('frontend')->render());
    }
...
//				    $valueToFilter = array('value' => $this->getRequest()->getPost('value'));
					$valueToFilter = $this->_filterDates($data, array('value'));
                    $value = date('Y-m-d', strtotime($valueToFilter['value']));
                } else 
                {
                    $value = null;
                }
