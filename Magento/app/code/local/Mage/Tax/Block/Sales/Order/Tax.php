...
class Mage_Tax_Block_Sales_Order_Tax extends Mage_Core_Block_Template
{
    ...
    protected function _initSubtotal()
    {
		...
		$disc = 0;
		$discIncTax = 0;
		if($this->_source->getDiscountAmount() != 0){
			$disc = $this->_source->getDiscountAmount();
			
			$discIncTax = $this->_source->getDiscountAmount();
			if($disc > 0){
				$disc = -1*$disc;
				$discIncTax = -1 * $discIncTax;
			}
			$disc += $this->_source->getHiddenTaxAmount();
        ...
        if ($this->_config->displaySalesSubtotalBoth($store)) {
            ...
            if ($total) {
                $total->setValue(max(0, $subtotalIncl+$discIncTax));
