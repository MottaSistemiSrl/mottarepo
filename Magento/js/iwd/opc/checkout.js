...
IWD.OPC = {
		...
		validatePayment: function(){	
			...
			if (paymentMethodForm.validator.validate()){				
				IWD.OPC.savePayment();
			}else{
		...
		saveOrder: function(){
			...
			IWD.OPC.Checkout.showLoader();
			IWD.OPC.Checkout.xhr = $j.post(IWD.OPC.Checkout.config.baseUrl + 'onepage/json/saveOrder',form,IWD.OPC.prepareOrderResponse,'json');
		},
		...
		init: function(){
			...
			$j('input[name="billing[create_account]"]').click(function(){
				if ($j(this).is(':checked')){
                    ...
                    //Dan Nistor - start
                    $j('div[class="fieldset forum hidden"]').removeClass('hidden');
                    $j('input[name="amcustomerattr[sevenlike_nickname]"]').addClass('required-entry');
                    $j('input[name="amcustomerattr[sevenlike_city]"]').addClass('required-entry');
                    $j('input[name="amcustomerattr[sevenlike_nation]"]').addClass('required-entry');
                    //Dan Nistor - show forum attributes - end
				}else{
                    ...
                    //Dan Nistor - show forum attributes - start
                    $j('div[class="fieldset forum"]').addClass('hidden');
                    $j('input[name="amcustomerattr[sevenlike_nickname]"]').removeClass('required-entry');
                    $j('input[name="amcustomerattr[sevenlike_city]"]').removeClass('required-entry');
                    $j('input[name="amcustomerattr[sevenlike_nation]"]').removeClass('required-entry');
                    //Dan Nistor - end
				}
			});
