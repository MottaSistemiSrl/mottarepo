...
if(typeof IWD.OPC != 'undefined'){
    IWD.OPC.prepareOrderResponse = function (response){
        ...
        if(!BancaSellaForm.lock && ($(BancaSellaForm.radioBancaSella).checked)){
            IWD.OPC.prepareExtendPaymentForm();
            $j('.opc-col-right').find('h3').first().text(Translator.translate('Attendi...'));
            $j('#opc-review-block').html('<div><p>'+Translator.translate('Attendi il completamento del pagamento')+'</p></div>');
    ...
    IWD.OPC.preparePaymentResponse = function(response){
        ...
        // fix per pagamento con iframe bancasella
        if(typeof(response.encrypt_string) != "undefined" ){
            ...
            //fix se per prodotti gratuiti
            if(typeof(response.free) != "undefined" && IWD.OPC.saveOrderStatus === true ){
                IWD.OPC.saveOrder();
            }
        }

        if (IWD.OPC.saveOrderStatus!==true){
            //quando è true allora viene gestito da createPagePaymentToOrder
            IWD.OPC.Plugin.dispatch('savePaymentAfter');
        }
